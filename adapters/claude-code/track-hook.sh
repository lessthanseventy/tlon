...
# promotes this session's thread into the stage machine via funes' track_thread, so the ticket
# condenses out of the work. Thin wrapper like heartbeat-hook.sh: the logic is cc-track.ts
# (bun, shares activity.ts's isCommitCommand + mcp.ts's FunesClient with pi's side). Never
