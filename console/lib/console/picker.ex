defmodule Console.Picker do
  @moduledoc """
  The two overlays of UX slice 2, which are one thing wearing two corpora: a **query**, a
  **cursor**, and a filtered list you pick from.
  ...
  Both read a corpus that already exists — the switcher off the rail's own `sidebar` read, so it
  costs no server call and can never disagree with what the rail is showing; the palette off a
  static table. `entries/2` filters and ranks with `Console.Fuzzy`.
