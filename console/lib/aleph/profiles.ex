...
defmodule Console.Profiles do
  ...
  Materialised layout (`PI_CODING_AGENT_DIR`):
      ...
      ~/.pi/profiles/<name>/
        ...
        tmux.conf         profile-specific — the persistence-free config its private tmux
                          server (`tmux -L aleph-<name>`) boots from (see `socket/1`)
  ...
  # Every coworker runs on its OWN tmux server (socket `aleph-<profile>`), booted from the
  ...
  # the chrome-off options the launcher used to chain on (aleph draws the Tlön header itself).
  @coworker_tmux_conf """
  ...
  (title, lead, brief) and the board stands the worker up as a proper funes citizen — its own
  ...
  @reviewer_role """
  You are {{handle}}, a code REVIEWER on this funes task thread. Read the diff you are asked to
  ...
  @planner_role """
  You are {{handle}}, an implementation PLANNER on this funes task thread. You turn a rough goal
  ...
  @builder_role """
  You are {{handle}}, an implementation BUILDER on this funes task thread. You work strictly
  ...
  @researcher_role """
  You are {{handle}}, a deep RESEARCHER on this funes task thread. Answer by fanning out
  ...
  # The seed roster — the name-keyed coworker instances aleph materialises today, expressed as
  ...
  @doc "The names of the coworker profiles aleph materialises — the rows the settings modal controls."
  @spec names() :: [String.t()]
  ...
  string}`. Read tolerantly: funes JSON (`%{"archetype" => "builder", "name" => "hronir"}`) or a
  ...
  @doc """
  The funes handles of a roster's WORKER (non-meta, registry-known) entries — `"<name>-machine"`.
  ...
  # The funes handle convention: profile name + "-machine" (profile "tertius" → "tertius-machine").
  ...
  @doc """
  The tmux socket name a coworker's private server runs on (`tmux -L aleph-<profile>`). One
